...
def print_comparison_report(comparison, limit=100):
    ...
    print("Comparing our values vs external rankings (harryknowsball + FantraxHQ)")
    print("="*120)

    print(f"\n{'Rank':<5} {'Player':<25} {'Pos':<8} {'Our Val':<10} {'Harry Norm':<12} {'Diff':<10} {'Diff %':<10} {'Assessment':<15}")
    print("-"*120)
    ...
    for p in comparison[:limit]:
        ...
        fantrax_str = f"(FHQ #{p['fantrax_rank']})" if p['fantrax_rank'] else ""

        print(f"{p['harry_rank']:<5} {p['name']:<25} {p['position']:<8} {p['our_value']:<10.1f} {p['harry_normalized']:<12.1f} {p['diff']:<+10.1f} {p['diff_pct']:<+10.1f}% {assessment:<15} {fantrax_str}")
